-- Version 1.0, too large number of lines on joins, need to optimize 
...
SELECT oh.order_key,sod.line_no AS line_number,p.product_key,sod.qty AS quantity,
	   ...
	   cr.currency_rate, sod.price * cr.currency_rate AS unit_price_usd,
	   sod.unit_cost * cr.currency_rate AS unit_cost_usd,
	   sod.qty * sod.price * cr.currency_rate AS sales_value_usd,
	   sod.qty * sod.unit_cost * cr.currency_rate AS sales_cost_usd,
	   sod.qty * (sod.price - sod.unit_cost) * cr.currency_rate AS margin_usd,	   
	   2 AS source_system_code,
	   soh.created AS create_timestamp, soh.last_updated AS update_timestamp
FROM Stage.dbo.order_detail sod
	...
	INNER JOIN Stage.dbo.order_header soh ON sod.order_id = oh.order_id 
